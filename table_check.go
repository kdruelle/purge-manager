...
func (t * Table) CheckIndexes(p * Table) (error) {
    query := fmt.Sprintf("EXPLAIN SELECT t.* FROM %s AS t", t.Name)
    if p != nil {
        query = fmt.Sprintf("%s INNER JOIN %s AS p ON %s", query, p.Name, t.Join)
    }
    if t.Condition != "" {
        ...
        for rows.Next() {
            ...
            if string(row["table"].([]byte)) == "p" {
                if row["possible_keys"] == nil {
                    log.Printf("%s : NO_INDEX_USED : %s", t.Name, query)
