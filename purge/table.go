...
func NewTable(tc config.TableConfig, conn *sql.DB, db string, softDelete bool) *Table {
	t := &Table{
		Conn:        conn,
	...
	for _, rtc := range tc.Related {
		rt := NewTable(rtc, conn, db, softDelete)
...
func (t *Table) populatePurgeScript(p *Table) int64 {
	var ra int64 = 0
	script := script.NewScript(t.Script)
	defer script.Close()
...
func (t *Table) selectDataToDelete() (data []map[string]interface{}) {
	...
	defer rows.Close()
	for rows.Next() {
		data = append(data, utils.ScanRow(rows))
	}
	return
}
